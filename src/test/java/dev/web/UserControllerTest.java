...
class UserControllerTest extends HttpServlet {
	 ...
	 * logIn: 로그인시 동작
	 * 
	 * 1. userid, passwd 정보 받기 2. DB에서 user 정보 조회 (만일 정보가 없을 경우, request "error"에 오류
	 * 메시지 저장) 3. session "login"에 해당 유저 정보를 저장 4. index.html로 이동 >> forward: /
	 */
	 ...
	 *
	 * 1. session에 있는 login 정보를 받기 2. listVideo를 통해 UserVideo table에서 해당 유저가 포함된
	 * 레코드들을 불러와 list에 저장 3. request "videoLog"에 list를 저장 >> profile.jsp를 실행
	 */
	 ...
	 * 
	 * 1. session에서 login정보를 받기 2. 해당 비디오 정보 받기 3. UserVideo table에 각각의 id 담기 4. 비디오
	 * 조회수 1증가 >> forward: /retrieve
	 */
